...
let rec kterm_to_term (kt : KTerm.term) : Term.term =
  ...
  | KTerm.Lam (ty_arg, body) ->
      {
        inner = Term.Fun (None, kterm_to_term ty_arg, kterm_to_term body);
        loc = Term.dummy_range;
      }
  | KTerm.Forall (ty_arg, ty_ret) ->
      {
        inner = Term.Arrow (None, kterm_to_term ty_arg, kterm_to_term ty_ret);
        loc = Term.dummy_range;
      }
  | KTerm.App (f, arg) ->
      { inner = Term.App (kterm_to_term f, kterm_to_term arg); loc = Term.dummy_range }
