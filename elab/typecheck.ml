...
let rec term_to_string (e : ctx) (tm : term) : string =
  ...
  | Hole idx -> (
      ...
      | Some { sol = Some tm_sol; _ } ->
          "(hole " ^ string_of_int idx ^ " = " ^ term_to_string e tm_sol ^ ")"
      | _ -> "Hole(" ^ string_of_int idx ^ ")")
  | Fun (arg, ty_arg, body) ->
      let arg_str = match arg with Some a -> a | None -> "_" in
      "(fun (" ^ arg_str ^ " : " ^ term_to_string e ty_arg ^ ") => "
      ^ term_to_string e body ^ ")"
  | Arrow (arg, ty_arg, ty_ret) ->
      let arg_str = match arg with Some a -> a | None -> "_" in
      "(" ^ arg_str ^ " : " ^ term_to_string e ty_arg ^ " -> "
      ^ term_to_string e ty_ret ^ ")"
  | App (f, arg) -> "(" ^ term_to_string e f ^ " " ^ term_to_string e arg ^ ")"
...
let valid_pattern_args (args : term list) : bool =
  if List.exists (fun arg -> match arg with Fvar _ -> false | _ -> true) args
  then false
  else
...
let rec valid_pattern (e : ctx) (m : int) (args : term list) (tm : term) : bool
  ...
  | Hole m' -> (
      if m = m' then (*print_endline "hole contains itself";*) false
        ...
        | _ -> true)
  | Fun (_, ty, body) ->
      valid_pattern e m args ty && valid_pattern e m args body
  | Arrow (_, ty, ret) ->
      valid_pattern e m args ty && valid_pattern e m args ret
...
let rec pattern_match_meta (e : ctx) (m : int) (args : term list) (tm : term) :
    unit =
  ...
  if List.length (Hashtbl.find e.metas m).vartypes < List.length args then
  ...
  else if not (valid_pattern_args args) then
    print_endline "invalid arguments for pattern matching"
    ...
    Hashtbl.replace e.metas m
      {
        (Hashtbl.find e.metas m) with
        sol =
          Some
            (fold tm (List.rev args)
               (List.rev (Hashtbl.find e.metas m).vartypes));
      }
...
let rec unify (e : ctx) (t1 : term) (t2 : term) : unit =
  ...
  | MetaSpine (Hole m1, args1), MetaSpine (Hole m2, args2) ->
      ...
      if List.length args1 != List.length args2 then
        ...
        Hashtbl.replace e.metas m1
          { (Hashtbl.find e.metas m1) with sol = Some (Hole m2) };
        List.iter2 (fun arg1 arg2 -> unify e arg1 arg2) args1 args2
  | MetaSpine (Hole m, args), _ -> pattern_match_meta e m args t2
  | _, MetaSpine (Hole m, args) -> pattern_match_meta e m args t1
  ...
  | _ ->
      failwith
        ("failed to unify non-matching terms " ^ term_to_string e t1 ^ " and "
       ^ term_to_string e t2)
...
let rec checktype (e : ctx) (tm : term) (ty : term) : unit =
  ...
  | App (f, arg) -> (
      try unify e ty (infertype e tm)
      with InferHole ->
        let argtype = infertype e arg in
        checktype e f (Arrow (None, argtype, ty)))
  | Name _ | Fun _ | Arrow _ | Sort _ | Fvar _ -> unify e ty (infertype e tm)
...
and check_is_type (e : ctx) (tm : term) : unit =
  ...
  | Hole m -> (
      match Hashtbl.find_opt e.metas m with
      | Some { ty = Some ty; _ } ->
          if not (is_sort ty) then failwith "expected hole to be a sort" else ()
      | _ -> ())
  | Name name -> (
      match Hashtbl.find_opt e.env name with
      | Some entry ->
          if not (is_sort entry.ty) then
            failwith ("expected " ^ name ^ " to be a type")
          else ()
      | None -> failwith ("unknown identifier: " ^ name))
  | Fun _ -> failwith "a function cannot be a type"
  | Arrow (arg, ty_arg, ty_ret) ->
      check_is_type e ty_arg;
      let x = gen_fvar_id () in
      let ty_ret_fvar = replace_bvar ty_ret 0 (Fvar x) in
  ...
  | App (f, arg) -> (
      ...
      print_endline ("because f is " ^ term_to_string e f ^ " and has type " ^ term_to_string e f_type); *)
          ...
          if not (is_sort ty_ret) then
            failwith "expected return type of function to be a sort"
  ...
  | Fvar fvar -> (
      ...
      | Some (_, ty) ->
          ...
          if not (is_sort ty || match ty with Hole _ -> true | _ -> false)
          then
            failwith
              ("expected type " ^ term_to_string e ty
             ^ " of free variable to be a sort")
          else ()
  ...
  | Hole m -> (
      ...
      | Some { sol = Some tm_sol; _ } ->
          if contains_fvar tm_sol then
            failwith "hole contains free variables, should have been bound"
...
let rec hole_to_meta (e : ctx) (stack : term list) (tm : term) : term =
  ...
  | Hole m ->
      ...
      let rec fold (tm : term) (level : int) : term =
        match level with 0 -> tm | n -> fold (App (tm, Bvar (n - 1))) (n - 1)
      in
...
let process_decl (e : ctx) (d : declaration) : unit =
  ...
  | Theorem (name, ty, proof) ->
      if Hashtbl.mem e.env name then
        ...
        else failwith ("invalid proof of " ^ name ^ ".\n")
  | Axiom (name, ty) ->
      if Hashtbl.mem e.env name then
        ...
        (* conv_to_kterm does a straightforward variant-to-variant conversion *)
        let ty_k = conv_to_kterm ty_filled in
        Hashtbl.add e.env name { name; ty = ty_filled; data = Axiom };
        Hashtbl.add e.kenv name ty_k
