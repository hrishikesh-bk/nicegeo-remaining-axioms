...
let isDefEq (env : environment) (localCtx : localcontext) (t1 : term)
    (t2 : term) : bool =
 ...
 *)
let rec inferType (env : environment) (localCtx : localcontext) (t : term) :
    term =
  match t with
  | Const name -> (
  ...
  | Forall (domainType, returnType) -> (
      ...
      let returnTypeType =
        ...
        let newLocalCtx =
          ...
          t
        in
        let substed_return_type =
          subst_bvar returnType 0 (Fvar new_fvar_name)
        in
