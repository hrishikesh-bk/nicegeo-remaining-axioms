...
type error_context = {
  ...
  term_name : string option;
      (*term_name - name of the term that caused the error*)
  expected_type : string option;
      (*expected_type - the expected type of the term*)
  actual_type : string option;
      (*actual_type - the actual inferred type of the term*)
  local_context : (string * string) list;
      (*local_context - the local context of the term*)
...
let raise_type_error ?(error_context = empty_context) msg =
  raise (TypeError (msg, error_context))

let raise_unknown_constant ?(error_context = empty_context) name =
  raise (UnknownConstant (name, error_context))

let raise_unknown_variable ?(error_context = empty_context) name =
  raise (UnknownVariable (name, error_context))

let raise_invalid_proof ?(error_context = empty_context) msg =
  raise (InvalidProof (msg, error_context))

let raise_parse_error ?(error_context = empty_context) msg =
  raise (ParseError (msg, error_context))

let raise_elab_error ?(error_context = empty_context) msg =
  raise (ElaboratorError (msg, error_context))

let raise_kernel_error ?(error_context = empty_context) msg =
  raise (KernelError (msg, error_context))

let raise_internal ?(error_context = empty_context) msg =
  raise (InternalError (msg, error_context))

let merge_ctx (newc : error_context) (oldc : error_context) : error_context =
  {
    loc = (match newc.loc with Some _ -> newc.loc | None -> oldc.loc);
    decl_name =
      (match newc.decl_name with
      | Some _ -> newc.decl_name
      | None -> oldc.decl_name);
    term_name =
      (match newc.term_name with
      | Some _ -> newc.term_name
      | None -> oldc.term_name);
    expected_type =
      (match newc.expected_type with
      | Some _ -> newc.expected_type
      | None -> oldc.expected_type);
    actual_type =
      (match newc.actual_type with
      | Some _ -> newc.actual_type
      | None -> oldc.actual_type);
    local_context =
      (if newc.local_context <> [] then newc.local_context
       else oldc.local_context);
...
let pp_kv k = function None -> "" | Some v -> Printf.sprintf "%s: %s\n" k v
...
let pp_ctx (c : error_context) =
  ...
  loc_line ^ pp_kv "decl" c.decl_name ^ pp_kv "term" c.term_name
  ^ pp_kv "expected" c.expected_type
  ^ pp_kv "actual" c.actual_type
  ^ pp_locals c.local_context ^ pp_kv "note" c.note
...
let fail_type ?(error_context = empty_context) msg =
  raise (TypeError (msg, error_context))

let fail_kernel ?(error_context = empty_context) msg =
  raise (KernelError (msg, error_context))

let fail_elab ?(error_context = empty_context) msg =
  raise (ElaboratorError (msg, error_context))
